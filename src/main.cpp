...
int main(int argc, char *argv[]) {
  ...
  dolfin::parameter::parameters.parse(argc, argv);
  ...
  // Default parameters
  dolfin::parameter::Parameters application_parameters(
  ...
  dolfin::parameter::parameters["mesh_partitioner"] = "SCOTCH";
  ...
  dolfin::common::Timer whole_program("[PERFORMANCE] Whole Application");
  ...
  if (problem_type == "poisson") {
    dolfin::common::Timer t0("[PERFORMANCE] Create Mesh");
  ...
  // Solve
  dolfin::common::Timer t5("[PERFORMANCE] Solve");
  std::size_t num_iter = solver.solve(*u->vector(), *b);
  t5.stop();

  if (output) {
    dolfin::common::Timer t6("[PERFORMANCE] Output");
    //  Save solution in XDMF format
    std::string filename =
        output_dir + "/solution-" + std::to_string(num_processes) + ".xdmf";
    dolfin::io::XDMFFile file(filename);
    file.write(*u);
    t6.stop();
  }
  whole_program.stop();
  ...
  dolfin::Table t =
      dolfin::timings(TimingClear::clear, {dolfin::TimingType::wall});
  ...
  if (dolfin::MPI::rank(mesh->mpi_comm()) == 0) {
    ...
    std::vector<std::string> timers{
        "[PERFORMANCE] Whole Application", // first timer is used for result
                                           // section
        "[PERFORMANCE] Solve",
    ...
    report.put_result("num_iter", num_iter)
        .put_result(HPCREPORT_RESULT_WALL_AVG,
                    t.get_value(timers[0], "wall avg"))
        .put_result(HPCREPORT_RESULT_WALL_MAX,
                    t_max.get_value(timers[0], "wall avg"))
        .put_result(HPCREPORT_RESULT_WALL_MIN,
                    t_min.get_value(timers[0], "wall avg"));
    ...
    report.add_lib("petsc").put(HPCREPORT_LIBRARY_VERSION, petsc_version);
